...
class MainFragmentViewModel @Inject constructor(
    ...
    fun insertCategories(categories: List<CategoryDto>) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            categories.maxByOrNull { it.updateTime }
                ?.let { updateSharedPref("categoryUpdateTime", it.updateTime) }
        }
    }

    fun insertCoins(coins: List<Coin>) = viewModelScope.launch(Dispatchers.IO) {
        roomRepository.insertCoins(coins)
        coins.maxByOrNull { it.updateTime }
            ?.let { updateSharedPref("coinUpdateTime", it.updateTime) }
    }
    ...
    private fun getNewCategories() = viewModelScope.launch(Dispatchers.IO) {
        ...
        apiRepository.getNewCategories(categoryUpdateTime).collect {
